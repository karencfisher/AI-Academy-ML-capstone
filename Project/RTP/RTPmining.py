...
def recent_temporal_pattern(mss, p, g):
	...
	'''
	mapping = []
	contains, mapping = ta.MSS_contains_Pattern(mss, p, 0, 0, mapping)
	if not contains:
		return False
	if not ta.recent_state_interval(mss, 
		   ta.get_index_in_sequence(mss, mapping[len(p.states)-1]), g):
		return False
	...
	for p in kRTP:
		for s in p_states:
			...
			for q in range(0, len(C)):
				C[q].p_RTPlist = ta.sequences_containing_state(p.RTPlist, s)
...
def pattern_mining(D, g, support, logger):
	'''
	The main RTP mining procedure that takes MSS of a group and their corresponding parameters and extracts ALL RTPs
	'''
	one_RTP = []
	freq_states = ta.find_all_frequent_states(D, support)
	logger.info(f'number of frequent states are: {len(freq_states)}')
	for s in freq_states:
		new_pattern = TemporalPattern([s],[['o']],[],[])
		RTPlist = []
		for Z in D:
			interval_matches = ta.find_state_matches(Z, s, 0)
			if len(interval_matches) != 0:
				if ta.recent_state_interval(Z, max(interval_matches), g):
					RTPlist.append(Z)
	...
	logger.info(f'the number of one-RTPs: {len(one_RTP)}')
	...
	for k in range(1, K+1):
		...
		logger.info(f'-----------length of the candidates for {k+1} pattern is: {len(candidates)}')
		kRTP = counting_phase(candidates, g, support)
		logger.info(f'-----------length of the kRTP for {k+1} pattern is: {len(kRTP)}')
		if len(kRTP) == 0:
			break
		Omega.extend(kRTP)
	logger.info(f'number of all patterns found: {len(Omega)}')
	return Omega
